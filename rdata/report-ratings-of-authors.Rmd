...
all.data <- data.frame(
  ...
  authorA=authora,comments2=comments1 ^ 2,
...
A linear model fit was done against multiple variables. The model notes the relationship between the number of comments on an article and the rating of the article. This model, while not ideal, was more suitable than the previous two. The difference between the ratings of articles belonging author A and the ratings of articles not belonging to author A is significant (p-value $< 0.001$).
...
The p-value `r pvalue` is less than $0.001$ (it is probably too small for float precision to properly display its value). There is very strong evidence against $H_0$. The difference between the rating-comment ratios of author A and other authors is significant. Due to the model not being appropriate, these results should be interpreted with skepticism.
...
The p-value for this hypothesis test is `r summary(model)$coefficients[2,4]` $< 0.001$. There is very strong evidence against $H_0$. The difference between the ratings of articles belonging to author A and the ratings of articles not belonging to author A is significant.